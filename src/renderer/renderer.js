...
const removeSecondarySelectedBtn = document.getElementById(
  "removeSecondarySelected"
);
...
function updateViewMode() {
  ...
  document
    ...
    .classList.toggle("hidden", isShowMode);
  document
    .getElementById("overlayTypeColor")
    .parentElement.classList.toggle("hidden", isShowMode);
  document
    .getElementById("overlayTypeImage")
    .parentElement.classList.toggle("hidden", isShowMode);
  document
    .getElementById("colorControl")
    .classList.toggle("hidden", isShowMode);
  document
    .getElementById("imageControl")
    .classList.toggle("hidden", isShowMode);
  document
    .querySelector(".global-mute-toggle")
    .classList.toggle("hidden", isShowMode);
  document.querySelectorAll(".arrow-buttons").forEach((el) => {
    el.classList.toggle("hidden", isShowMode);
    el.querySelectorAll("button").forEach((btn) => {
      btn.disabled = isShowMode;
      btn.style.pointerEvents = isShowMode ? "none" : "";
    });
  });

  sequenceList.style.pointerEvents = isShowMode ? "none" : "";
  secondaryMediaList.style.pointerEvents = isShowMode ? "none" : "";

  document
    .querySelectorAll("#sequenceList li, #secondaryMediaList li")
    .forEach((li) => li.setAttribute("draggable", String(!isShowMode)));

  document
    .querySelectorAll('label[for^="overlayType"], #colorControl, #imageControl')
  ...
  document
    .querySelector("#instantColumn .deck-controls")
    .classList.toggle("hidden", isShowMode);
}
...
function containerDragOver(e) {
  ...
  const children = Array.from(sequenceList.children).filter(
  ...
  dragPlaceholder.textContent = primaryLibrary[draggedIndex]
    .split(/(\\|\/)/g)
    .pop();
  ...
  primaryLibrary.forEach((item, index) => {
    ...
    if (index > 0) {
      ...
      upButton.addEventListener("click", (e) => {
        e.stopPropagation();
        [primaryLibrary[index - 1], primaryLibrary[index]] = [
          primaryLibrary[index],
          primaryLibrary[index - 1],
        ];
    ...
    if (index < primaryLibrary.length - 1) {
      ...
      downButton.addEventListener("click", (e) => {
        e.stopPropagation();
        [primaryLibrary[index], primaryLibrary[index + 1]] = [
          primaryLibrary[index + 1],
          primaryLibrary[index],
        ];
        if (currentIndex === index) {
...
function secondaryContainerDragOver(e) {
  ...
  const children = Array.from(secondaryMediaList.children).filter(
    (child) => !child.classList.contains("drag-placeholder")
  );
  ...
  secondaryDragPlaceholder.textContent = secondaryLibrary[secondaryDraggedIndex]
    .split(/(\\|\/)/g)
    .pop();
  ...
  if (newIndex >= secondaryMediaList.children.length) {
    ...
    secondaryMediaList.insertBefore(
      secondaryDragPlaceholder,
      secondaryMediaList.children[newIndex]
    );
  }
}
...
function secondaryContainerDrop(e) {
  ...
  let newIndex =
    secondaryDragNewIndex !== null
      ? secondaryDragNewIndex
      : secondaryLibrary.length;
  if (draggedIdx < newIndex) newIndex--;
  if (draggedIdx === newIndex) return;
...
document.addEventListener("dragend", () => {
  ...
  document
    .querySelectorAll(".secondary-dragging")
    .forEach((item) => item.classList.remove("secondary-dragging"));
});
//#endregion
    ...
    if (index > 0) {
      ...
      up.addEventListener("click", (e) => {
        ...
        [secondaryLibrary[index - 1], secondaryLibrary[index]] = [
          secondaryLibrary[index],
          secondaryLibrary[index - 1],
        ];
        updateLibraryUI();
      });
...
nextBtn.addEventListener("click", () => {
  ...
  const isAtEnd =
    parseFloat(timeSlider.value) >= parseFloat(timeSlider.max) - 0.1;
  ...
  if (isAtStart && !isPlaying) {
    ...
    if (nextButtonClickTimeout) {
      ...
      if (
        ...
        currentIndex < primaryLibrary.length - 1
      ) {
        currentIndex++;
        ipcRenderer.send("load-video-preview", primaryLibrary[currentIndex]);
        isPlaying = false;
...
timeSlider.addEventListener("input", () => {
  ...
  timeDisplay.textContent = `${formatTime(previewTime)} / ${formatTime(
    timeSlider.max
  )}`;
});
...
removeSecondarySelectedBtn.addEventListener("click", () => {
  if (selectedSecondaryFile) {
    secondaryLibrary = secondaryLibrary.filter(
      (item) => item !== selectedSecondaryFile
    );
    selectedSecondaryFile = null;
    renderSecondaryMedia();
  } else {
...
playSecondaryBtn.addEventListener("click", () => {
  ...
  secondaryAudio.addEventListener("loadedmetadata", () => {
    ...
    secondaryTimeSlider.value = 0;
    secondaryTimeDisplay.textContent = `0:00 / ${formatTime(
      secondaryAudio.duration
    )}`;
  });
  ...
  secondaryAudio.addEventListener("timeupdate", () => {
    if (!isSeekingSecondary) {
      ...
      secondaryTimeDisplay.textContent = `${formatTime(
        secondaryAudio.currentTime
      )} / ${formatTime(secondaryAudio.duration)}`;
    }
  });
...
secondaryTimeSlider.addEventListener("input", () => {
  ...
  secondaryTimeDisplay.textContent = `${formatTime(t)} / ${formatTime(
    secondaryTimeSlider.max
  )}`;
});
...
instantVolumeIcon.addEventListener("click", () => {
  if (instantGlobalVolume > 0) {
    ...
    instantGlobalVolume =
      parseFloat(globalVolSlider.getAttribute("data-previous")) || 1;
...
globalVolSlider.addEventListener("input", () => {
  ...
  instantVolumeIcon.classList.remove(
    "fa-volume-mute",
    "fa-volume-low",
    "fa-volume-high"
  );
